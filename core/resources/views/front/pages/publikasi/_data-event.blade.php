                ...
                <article class="entry entry-custom">
                    <div class="ipost clearfix">
                        <div class="entry-image">
                            <a href="{{ asset('file_app/event_image/'.$beritas->media) }}"
                            data-lightbox="image"><img class="image_fade custom-img"
                                src="{{ asset('file_app/event_image/'.$beritas->media) }}"></a>
                        </div>
                        <div class="entry-title">
                            <h3><a href="{{ route('publikasi.event.detail',$beritas->slug) }}">{{ Session::has('locale') ? Session::get('locale') == 'id' ? $beritas->judul_id : ( (!empty($beritas->judul_en)) ? $beritas->judul_en : $beritas->judul_id ) : $beritas->judul_id }}
                            </a></h3>
                        </div>
                        <ul class="entry-meta clearfix">
                            <li><i class="icon-calendar3"></i> {{ date('d M Y',strtotime($beritas->tanggal_awal)) }} - {{ date('d M Y',strtotime($beritas->tanggal_awal)) }}</li>
                        ...
                        <a href="{{ route('publikasi.event.detail',$beritas->slug) }}" class="more-link mt-3" style="color: #007dc2;">Read More</a>
                    </div>
                </article>
            </div>
